...
public class htmlLayout extends org.apache.log4j.Layout {
	...
	public String format(LoggingEvent event) {
		...
		/*
		 * sbuf.append("<td>"); sbuf.append(event.timeStamp -
		 * LoggingEvent.getStartTime()); sbuf.append("</td>" + Layout.LINE_SEP);
		 */
		String msg = event.getRenderedMessage();
		if (msg.startsWith("Row Number")) {
			String escapedThread = Transform.escapeTags(event.getThreadName());
			sbuf.append("<td bgcolor=\"#000000\" title=\"" + escapedThread + " Property Name\">");
			sbuf.append("</td>" + Layout.LINE_SEP);

			sbuf.append("<td bgcolor=\"#000000\" title=\"SIT\">");
			sbuf.append("</td>" + Layout.LINE_SEP);

			sbuf.append("<td bgcolor=\"#000000\" title=\"DataBase\">");
			sbuf.append("</td>" + Layout.LINE_SEP);

			sbuf.append("<td bgcolor=\"#000000\" title=\"Result\">");
			sbuf.append("</td>" + Layout.LINE_SEP);
		} else {
			String[] msgArr = new String[4]; // Change msgArr when u change the
												// info or error message
			StringTokenizer st2 = new StringTokenizer(msg, "$$");
			int i = 0;
			while (st2.hasMoreTokens()) {
				msgArr[i] = st2.nextToken();
				i++;
			}

			if (String.valueOf(event.getLevel()).equals("INFO")) {
				String escapedThread = Transform.escapeTags(event.getThreadName());
				sbuf.append("<td title=\"" + escapedThread + " Property Name\">");
				sbuf.append(msgArr[0]);
				sbuf.append("</td>" + Layout.LINE_SEP);
			} else {
				String escapedThread = Transform.escapeTags(event.getThreadName());
				sbuf.append("<td title=\"" + escapedThread + " Property Name\">");
			...
			if (String.valueOf(event.getLevel()).equals("INFO")) {
				sbuf.append("<td title=\"SIT\">");
				sbuf.append(Transform.escapeTags(String.valueOf(msgArr[1])));
				sbuf.append("</td>" + Layout.LINE_SEP);
			} else {
				sbuf.append("<td title=\"DataBase\">");
				sbuf.append("<font color=\"#ff0000\">");
				sbuf.append(Transform.escapeTags(String.valueOf(msgArr[1])));
				sbuf.append("</font>");
				sbuf.append("</td>" + Layout.LINE_SEP);
			...
			if (String.valueOf(event.getLevel()).equals("INFO")) {
				sbuf.append("<td title=\"SIT\">");
				sbuf.append(Transform.escapeTags(msgArr[3]));
			} else {
				sbuf.append("<td title=\"SIT\">");
				sbuf.append("<font color=\"#ff0000\">");
				sbuf.append(Transform.escapeTags(msgArr[3]));
			...
			if (String.valueOf(event.getLevel()).equals("INFO")) {
				if (msg.startsWith("Row Number")) {
					...
					sbuf.append("<td title=\"Result\">");
					sbuf.append("<font color=\"#008000\">");
					sbuf.append(Transform.escapeTags("PASS"));
					sbuf.append("</font>");
				}
			} else {
				sbuf.append("<td title=\"Result\">");
				sbuf.append("<font color=\"#ff0000\">");
				sbuf.append(Transform.escapeTags("FAIL"));
				sbuf.append("</font>");
			}
			sbuf.append("</td>" + Layout.LINE_SEP);
		}
	...
	public String getHeader() {
		...
		sbuf.append("<th>Property Name</th>" + Layout.LINE_SEP);
		sbuf.append("<th>SIT</th>" + Layout.LINE_SEP);


		if (locationInfo) {
			sbuf.append("<th>File:Line</th>" + Layout.LINE_SEP);
		}
		sbuf.append("<th>DataBase</th>" + Layout.LINE_SEP);
		sbuf.append("<th>Result</th>" + Layout.LINE_SEP);
		sbuf.append("</tr>" + Layout.LINE_SEP);
		return sbuf.toString();
	}
